...
close all
load csca_Aggregate_Si_65nm_0_XY % csca from SIE

%%%%% input %%%%%
wl_interp = (400:2:650).'; % make sure that wl is consistent with the csca
N_multipole = 2;
%%%%%%%%%%%%%%%%%
...
m = abs((1i).*m_x+m_y).^2./2+abs(m_z).^2+...
    abs((1i).*m_x-m_y).^2./2;
...
qE_yz = D_0.*(a_n_kuadrupol(:,ind_p1)+a_n_kuadrupol(:,ind_m1));
qE_yx = qE_xy;
qE_zx = qE_xz;
qE_zy = qE_yz;
...
qM_yz = D_0.*c.*(b_n_kuadrupol(:,ind_p1)+b_n_kuadrupol(:,ind_m1));
qM_yx = qM_xy;
qM_zx = qM_xz;
qM_zy = qM_yz;

%multipole scattering cross section
c_sca_p = (1./(k.^2.*C_0.^2)).*(abs((1i).*p_x+p_y).^2./2+abs(p_z).^2+...
    abs((1i).*p_x-p_y).^2./2);
c_sca_m = (1./(k.^2.*(c^2*C_0.^2))).*(abs((1i).*m_x+m_y).^2./2+abs(m_z).^2+...
    abs((1i).*m_x-m_y).^2./2);
c_sca_qE = (6./(4.*k.^2.*D_0.^2)).*(abs(2.*qE_xx+2.*(1i).*qE_xy-qE_zz).^2./4+...
    abs(qE_xz+(1i).*qE_yz).^2+abs((1i).*qE_yz-qE_xz).^2+...
    abs(2.*qE_yy-2.*(1i).*qE_xy+qE_zz).^2./4+2/3.*abs(qE_zz).^2);
c_sca_qM = (6./(4.*k.^2.*(c.^2.*D_0.^2))).*(abs(2.*qM_xx+2.*(1i).*qM_xy+qM_zz).^2./4+...
    abs(qM_xz+(1i).*qM_yz).^2+abs((1i).*qM_yz-qM_xz).^2+...
    abs(2.*qM_yy-2.*(1i).*qM_xy+qM_zz).^2./4+2/3.*abs(qM_zz).^2);
c_sca_multipole = c_sca_p + c_sca_m + c_sca_qE + c_sca_qM;

%plotting scattering cross section
figure()
plot(lamda2,abs(c_sca_num),'','color',[0 0 0],'LineWidth',2)
hold on
plot(lamda2,abs(c_sca_multipole),'--','color',[0 0 0],'LineWidth',2)
hold on
plot(lamda2,abs(c_sca_p),'','color',[0 0 1],'LineWidth',2)
hold on
plot(lamda2,abs(c_sca_m),'--','color',[0 0 1],'LineWidth',2)
hold on
plot(lamda2,abs(c_sca_qE),'','color',[1 0 0],'LineWidth',2)
hold on
plot(lamda2,abs(c_sca_qM),'--','color',[1 0 0],'LineWidth',2)
ylabel('C_{sca} (m^2)')
ylim([0 3*10^(-13)])
xlabel('\lambda (nm)')
legend('c_{sca} SIE','c_{sca} multipole','c_{sca} p','c_{sca} m','c_{sca} qE','c_{sca} qM')

% save resonance_Si_65nm_0_XY.mat...
%     c_sca_p...
%     c_sca_m...
%     c_sca_qE...
%     c_sca_qM...
...
% ylabel('p_z')
% figure()
% plot(lamda2,abs(qE_xx),'LineWidth',2)
% ylabel('Q_{xx}')
% figure()
% plot(lamda2,abs(qE_yy),'LineWidth',2)
% ylabel('Q_{yy}')
% figure()
% plot(lamda2,abs(qE_zz),'LineWidth',2)
% ylabel('Q_{zz}')
% figure()
% plot(lamda2,abs(qE_xy),'LineWidth',2)
% ylabel('Q_{xy}')
% figure()
% plot(lamda2,abs(qE_xz),'LineWidth',2)
% ylabel('Q_{xz}')
% figure()
% plot(lamda2,abs(qE_yz),'LineWidth',2)
% ylabel('Q_{yz}')
% xlabel('\lambda (nm)')
